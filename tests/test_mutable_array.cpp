#include <iostream>
#include "../src/MutableArraySequence.h"
#include "../src/ImmutableArraySequence.h"
#include "../src/test_utils.h"

using namespace std;

void run_test_mutable_array() {
    reset_counters();

    cout << "=== MutableArraySequence Tests ===" << endl;

    //пустая последовательность
    MutableArraySequence<int>* seq1 = new MutableArraySequence<int>();
    check(seq1->GetLength() == 0, "empty: length = 0");
    delete seq1;

    //создание из массива
    int data[] = {10, 20, 30};
    MutableArraySequence<int>* seq2 = new MutableArraySequence<int>(data, 3);
    check(seq2->GetLength() == 3, "from array: length = 3");
    check(seq2->Get(0) == 10, "from array: first element");
    check(seq2->Get(2) == 30, "from array: last element");
    delete seq2;

    //append
    MutableArraySequence<int>* seq3 = new MutableArraySequence<int>();
    seq3 = (MutableArraySequence<int>*)seq3->Append(10);
    seq3 = (MutableArraySequence<int>*)seq3->Append(20);
    check(seq3->GetLength() == 2, "append: length = 2");
    check(seq3->GetFirst() == 10, "append: first");
    check(seq3->GetLast() == 20, "append: last");
    delete seq3;

    //prepend
    MutableArraySequence<int>* seq4 = new MutableArraySequence<int>();
    seq4 = (MutableArraySequence<int>*)seq4->Prepend(10);
    seq4 = (MutableArraySequence<int>*)seq4->Prepend(20);
    seq4 = (MutableArraySequence<int>*)seq4->Prepend(30);
    check(seq4->GetFirst() == 30, "prepend: first = 30");
    check(seq4->Get(1) == 20, "prepend: middle = 20");
    check(seq4->GetLast() == 10, "prepend: last = 10");
    delete seq4;

    //insertAt в начало
    MutableArraySequence<int>* seq5 = new MutableArraySequence<int>(data, 3);
    seq5 = (MutableArraySequence<int>*)seq5->InsertAt(99, 0);
    check(seq5->Get(0) == 99, "insertAt(0): inserted");
    check(seq5->Get(1) == 10, "insertAt(0): shift");
    check(seq5->GetLength() == 4, "insertAt(0): length");
    delete seq5;

    //insertAt в середину
    MutableArraySequence<int>* seq6 = new MutableArraySequence<int>(data, 3);
    seq6 = (MutableArraySequence<int>*)seq6->InsertAt(99, 1);
    check(seq6->Get(0) == 10, "insertAt(1): element 0");
    check(seq6->Get(1) == 99, "insertAt(1): inserted");
    check(seq6->Get(2) == 20, "insertAt(1): shift");
    delete seq6;

    //insertAt в конец
    MutableArraySequence<int>* seq7 = new MutableArraySequence<int>(data, 3);
    seq7 = (MutableArraySequence<int>*)seq7->InsertAt(99, 3);
    check(seq7->GetLast() == 99, "insertAt(end): inserted");
    check(seq7->GetLength() == 4, "insertAt(end): length");
    delete seq7;

    //getSubsequence
    int data2[] = {1, 2, 3, 4, 5};
    MutableArraySequence<int>* seq8 = new MutableArraySequence<int>(data2, 5);
    Sequence<int>* sub = seq8->GetSubsequence(1, 3);
    check(sub->GetLength() == 3, "getSubsequence: length");
    check(sub->Get(0) == 2, "getSubsequence: first");
    check(sub->Get(2) == 4, "getSubsequence: last");
    delete seq8;
    delete sub;

    //concat
    MutableArraySequence<int>* seq9 = new MutableArraySequence<int>(data, 2);
    MutableArraySequence<int>* seq10 = new MutableArraySequence<int>(data + 2, 1);
    Sequence<int>* concat = seq9->Concat(seq10);
    check(concat->GetLength() == 3, "concat: length");
    check(concat->Get(0) == 10, "concat: from first");
    check(concat->Get(2) == 30, "concat: from second");
    delete seq9;
    delete seq10;
    delete concat;

    //экстремальные значения
    MutableArraySequence<int>* seq11 = new MutableArraySequence<int>();
    seq11 = (MutableArraySequence<int>*)seq11->Append(-2147483648);
    seq11 = (MutableArraySequence<int>*)seq11->Append(0);
    seq11 = (MutableArraySequence<int>*)seq11->Append(2147483647);
    check(seq11->Get(0) == -2147483648, "extreme: int_min");
    check(seq11->Get(1) == 0, "extreme: zero");
    check(seq11->Get(2) == 2147483647, "extreme: int_max");
    delete seq11;

    //копирование
    MutableArraySequence<int>* seq12 = new MutableArraySequence<int>(data, 3);
    MutableArraySequence<int>* seq13 = new MutableArraySequence<int>(*seq12);
    check(seq13->GetLength() == 3, "copy: length");
    check(seq13->Get(0) == 10, "copy: data");
    delete seq12;
    delete seq13;

    cout << "\n=== ImmutableArraySequence Tests ===" << endl;

//пустая
    Sequence<int>* imm1 = new ImmutableArraySequence<int>();
    check(imm1->GetLength() == 0, "immutable empty: length = 0");
    delete imm1;

//append
    Sequence<int>* imm2 = new ImmutableArraySequence<int>();
    Sequence<int>* old_imm2 = imm2;
    imm2 = imm2->Append(10);
    delete old_imm2;

    old_imm2 = imm2;
    imm2 = imm2->Append(20);
    delete old_imm2;

    check(imm2->GetLength() == 2, "immutable append: length");
    check(imm2->GetFirst() == 10, "immutable append: first");
    check(imm2->GetLast() == 20, "immutable append: last");
    delete imm2;

//prepend
    Sequence<int>* imm3 = new ImmutableArraySequence<int>();
    Sequence<int>* old_imm3 = imm3;
    imm3 = imm3->Prepend(10);
    delete old_imm3;

    old_imm3 = imm3;
    imm3 = imm3->Prepend(20);
    delete old_imm3;

    check(imm3->GetFirst() == 20, "immutable prepend: first");
    check(imm3->GetLast() == 10, "immutable prepend: last");
    delete imm3;

//insertAt
    Sequence<int>* imm4 = new ImmutableArraySequence<int>(data, 3);
    Sequence<int>* old_imm4 = imm4;
    imm4 = imm4->InsertAt(99, 1);
    delete old_imm4;

    check(imm4->Get(1) == 99, "immutable insertAt: inserted");
    check(imm4->GetLength() == 4, "immutable insertAt: length");
    delete imm4;

    print_results();
}